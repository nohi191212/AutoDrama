...
| `role_extract` | `workflows/nodes/role_nodes.py` | `ProviderRouter.text("role")`, `RoleService.role_extract` | Script content helpers for novel text | `state.metadata.role_extract_*`, `state.budget.used_text_calls` | `assets/json/nodes/role_extract.json` |
...
| `prop_design` | `workflows/nodes/static_asset_nodes.py` | `ProviderRouter.text("prop")`, `AssetService.prop_design` | `PropDesignRepository` candidate; script content helpers | `state.props`, `state.budget.used_text_calls` | `assets/json/nodes/prop_design.json`; per-prop `assets/json/props/{prop_id}.json` |
| `prop_image_generation` | `workflows/nodes/static_asset_nodes.py` | `ProviderRouter.image("prop")` | `MediaStore.write_first_generated_image`, prop design update helpers, reference resolver candidate | `state.props[*].asset_*`, provider/model/request fields | `assets/json/nodes/prop_image_generation.json`; `assets/images/props/*` |
| `script_compress` | `workflows/nodes/static_asset_nodes.py` | `ProviderRouter.text("script")`, `AssetService.script_compress` | Script content helpers | `state.metadata.simple_script`, `state.metadata.global_script`, `state.budget.used_text_calls` | `assets/json/nodes/script_compress.json` |
| `layout_design` | `workflows/nodes/static_asset_nodes.py` | `ProviderRouter.text("layout")`, `AssetService.layout_design` | Script content helpers | `state.layouts`, `state.budget.used_text_calls` | `assets/json/nodes/layout_design.json` |
...
| A1 script node migration | `script_outline`, `script_novel`, and `script_novel_extract` are owned by `Script*Node` classes; `PregenWorkflow` wrappers remain for compatibility. |
