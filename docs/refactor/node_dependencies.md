...
| --- | --- | --- | --- | --- | --- |
| `script_import` | `workflows/nodes/script_nodes.py` (`ScriptImportNode`) | none (pure chapter-file import) | `ScriptChapter` loader, `ScriptContentRepository`, `ProjectRepository.save_node_output` | `state.raw_script`, `state.script.raw_script`, `state.script.novel_full`, `state.metadata.script_import_*` | `assets/json/nodes/script_import.json`; per-episode `assets/json/scripts/novel_full/{episode_key}.json` |
| `script_cinematic_adapt` | `workflows/nodes/script_nodes.py` (`ScriptCinematicAdaptNode`) | `ProviderRouter.text("script")`, `ScriptService.script_cinematic_adapt` | `ScriptContentRepository`, `ProjectRepository.save_node_output` | `state.script.novel_full`, `state.metadata.script_cinematic_adapted`, `state.budget.used_text_calls` | `assets/json/nodes/script_cinematic_adapt.json`; per-episode `assets/json/scripts/novel_full/{episode_key}.json` |
...
| `layout_finalize` | `workflows/nodes/static_asset_nodes.py` | `ProviderRouter.text("layout")`, `AssetService.layout_finalize` | none beyond node output repository | `state.layouts`, `state.budget.used_text_calls` | `assets/json/nodes/layout_finalize.json` |
...
| Script content loading | `_load_script_contents`, `_novel_full_contents` | `repositories/script_content_repo.py` | `ScriptContentRepository` now owns path and content JSON compatibility for script nodes; shared wrappers remain in `PregenWorkflow`. |
...
| A1 script node migration | `script_import`, `script_cinematic_adapt`, and `clip_segment` are default `Script*Node` classes; `script_outline` and `script_novel` remain manual optional nodes, and only `script_outline` satisfies the `script_import` completion alias; `key_vision_prompt` and `key_vision_image_generation` are inserted after `clip_segment`. |
| A2 role node migration | `role_finalize` is owned by `Role*Node` classes and writes final role records for downstream roleboard generation; legacy standalone role extract/audit nodes have been removed. |
