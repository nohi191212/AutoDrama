...
| A1 script node migration | `script_outline`, `script_novel`, `script_novel_extract`, and `clip_segment` are owned by `Script*Node` classes; `design_key_vision_prompt` and `design_key_vision_image` are inserted after `clip_segment`; `PregenWorkflow` wrappers remain for compatibility. |
