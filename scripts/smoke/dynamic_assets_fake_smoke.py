...
async def main_async() -> int:
    ...
    state = await workflow.run(project_dir, until="dynamic_asset_solidification", force=True)
    slot_path = project_dir / "slots" / "episode_001.json"
    ...
    if '"ref_frame_asset_path"' not in slot_text:
        raise AssertionError("slot missing ref_frame_asset_path")
    if '"video_asset_path"' not in slot_text:
        raise AssertionError("slot missing video_asset_path")
    if '"dialogue_audio_assets"' not in slot_text:
        raise AssertionError("slot missing dialogue_audio_assets")
