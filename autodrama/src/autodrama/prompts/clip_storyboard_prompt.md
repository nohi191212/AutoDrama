...
故事板图固定要求：
- 每个 clip 会生成一张完整故事板图。
- 每张故事板严格为 {{storyboard_grid}} = {{storyboard_panel_count}} 宫格，整张图为 1:1 方形画布。
- 每个宫格内部以 {{storyboard_panel_aspect_ratio}} 的画面区组织内容，宫格底部必须保留文字说明条。
...
- 故事板必须明显看出真实 camera shot 的切镜边界：在切镜发生的两个相邻宫格正中间分隔线上，画一条醒目的红色斜杠 cut mark，不要画在某个宫格的斜下角。若边界是 P04-P05 或 P08-P09 这种跨行连续编号，必须把红色斜杠画在两行之间的水平分隔线中央，并加小黑字 `CUT P04-P05` 或 `CUT P08-P09`。
...
- 每个 Camera Shot 边界必须在相邻 P 面板之间明确标注：`切镜标记：在 P03 与 P04 之间的正中间分隔线上画醒目的红色斜杠`。跨行边界必须写成：`切镜标记：在 P04 与 P05 所在两行之间的水平分隔线中央画醒目的红色斜杠，并标注 CUT P04-P05`。不要写成角落斜杠或某个面板右下角斜杠。
...
- 故事板宫格底部文字说明带必须额外写一行颜色图例文字，明确说明本宫格里各色箭头的含义，至少包含：红=动作，蓝=镜头，绿=构图，橙=灯光，紫=情绪/声音，黑=镜头注记。整张图底部再保留一条全局颜色图例说明带。
- 继承项目约束里的全局视觉风格和视听调性；具体剧情事实、镜头拆分和动作推进以当前集及相邻集完整正文、当前集剧情摘要、本批 clip 片段和资产摘要为准。
