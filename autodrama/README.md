...
`director_prep` runs after `script_novel` and before the visual/script extraction nodes. It reads the full `novel_full` set and writes `assets/json/nodes/director_prep.json`, containing only `story_core`, `worldview`, and `visual_tone`. Downstream text nodes use it as a compact director constraint reference, while `novel_full` remains the source of truth for script facts.
