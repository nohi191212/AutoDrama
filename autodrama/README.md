...
22. `clip_storyboard_prompt_audit`
23. `clip_storyboard_image_generation`
24. `clip_storyboard_keyframe_generation`
25. `clip_manifest_generation`
26. `role_subject_video_generation`
27. `role_subject_element_generation`
28. `role_voice_select`
29. `bgm_design`
30. `bgm_generation`
...
`clip_storyboard_prompt` also supports `--clips 1-3`, comma/range selectors, and full clip IDs. It submits only matching clips to the text provider and merges regenerated clips into the existing aggregate output without changing unselected clips.
...
`run generation` processes selected episodes in episode order. It reads existing `shots/{episode_key}.json` files, generates dialogue audio when a clip has dialogue, generates clip videos, and writes solidified dynamic asset metadata back into those clip records before moving to the next episode. Dialogue remains a structured field for TTS, but the spoken line still appears in `video_prompt`. `clip_manifest_generation` prepares the fixed `shot_video_inputs` list and model-specific `final_video_prompt`; `clip_video_generation` only submits those prepared inputs. In Kling subject mode, shot video generation uses `clip_start_frame`, `clip_end_frame`, the clip-level 12-panel storyboard sheet, then available layout and prop images; characters are submitted separately as Kling subject elements. Kling's `max_reference_images` limit applies to storyboard/layout/prop `refer_image` inputs and does not consume the dedicated first/last-frame anchors. First clip start/end both come from itself; later clips start from the previous clip end frame. Other video providers use storyboard-first context images followed by roleboard, layout, and prop images. It reads `generation_checklist.json` when present and supports `--episodes` to target specific episodes.
...
- Reference images are submitted as public URLs. Expired signed URLs are reported in light red, refreshed from the local file through ToAPI, and cached across CLI runs in `assets/json/cache/reference_image_urls.json`; `providers.toapi.options.toapi_reference_url_cache_ttl_seconds` controls the fallback validity window. RightCode no longer accepts local image files or inline base64.
...
D:/miniforge3/envs/autodrama/python.exe -m autodrama.cli run pregen --config config.yaml --project <project_id> --only clip_storyboard_prompt --episodes 1 --clips 1-3 --force
D:/miniforge3/envs/autodrama/python.exe -m autodrama.cli run pregen --config config.yaml --project <project_id> --only clip_storyboard_prompt_audit --episodes 1 --clips 1-8 --force
