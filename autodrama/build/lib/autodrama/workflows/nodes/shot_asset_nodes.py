...
from autodrama.core.schemas import (
    ...
    ShotBackgroundShotReferenceEpisodeOutput,
    ShotBackgroundShotReferenceItem,
    ShotBackgroundShotReferenceOutput,
...
class ShotAssetNodeBase(StaticAssetNodeBase):
    ...
    def _shot_reference_asset_ref(
        self,
        project_dir: Path,
        item: ShotBackgroundShotReferenceItem,
    ) -> AssetRef:
        path = project_dir / item.asset_path
        if not path.is_file() and not item.asset_url:
            raise FileNotFoundError(
                f"Shot reference image is missing for {item.shot_id}: {item.asset_path}"
            )
        return AssetRef(
            ...
            url=item.asset_url,
            metadata={
                "asset_type": "shot_background_shot_reference",
                "shot_id": item.shot_id,
    ...
    def _invalidate_shot_reference_dependents(
        self,
        project_dir: Path,
        state: ProjectState,
        episode_key: str,
        ...
        self._invalidate_background_dependents(project_dir, state, episode_key, shot_ids)
    ...
    def _validate_model_output(
        ...
        *,
        ...
        for _, item in ordered:
            item.scene_id = cls._clean(item.scene_id)
            ...
            if item.scene_id != scene_id:
                raise ValueError(
                    f"clip_to_shots must bind every shot to scene {scene_id}; got {item.scene_id}"
                )
            ...
            item.camera.scene_position = cls._clean(item.camera.scene_position)
            item.camera.target = cls._clean(item.camera.target)
            item.camera.shooting_angle = cls._clean(item.camera.shooting_angle)
            item.camera.shot_size = cls._clean(item.camera.shot_size)
            item.camera.movement = cls._clean(item.camera.movement)
            camera_text = "\n".join(
                (
                    item.camera.scene_position,
                    item.camera.target,
                    item.camera.shooting_angle,
                    item.camera.shot_size,
            ...
            if not all(
                ...
                raise ValueError("clip_to_shots requires a complete camera specification")
            visible_text = "\n".join(
                (item.shot_description, item.narrative_angle, item.opening_state, item.video_prompt, camera_text)
            ).lower()
            ...
            if len(item.ref_ids) != len(set(item.ref_ids)):
                raise ValueError("clip_to_shots ref_ids must not contain duplicates")
            ...
            scene_refs = [ref_id for ref_id in item.ref_ids if assets[ref_id]["kind"] == "layout"]
            if scene_refs:
                raise ValueError("scene references belong in scene_id and must not appear in ref_ids")
            if 1 + len(item.ref_ids) > reference_budget:
                raise ValueError(
                    f"clip_to_shots uses {1 + len(item.ref_ids)} total image references; "
                    f"budget is {reference_budget}"
                )
            referenced_role_ids = {
            ...
            for placement in item.character_placements:
                placement.role_id = cls._clean(placement.role_id)
                placement.scene_position = cls._clean(placement.scene_position)
                placement.screen_position = cls._clean(placement.screen_position)
                placement.body_facing = cls._clean(placement.body_facing)
                placement.pose = cls._clean(placement.pose)
                placement.gaze_target = cls._clean(placement.gaze_target) or None
                if placement.role_id not in valid_role_ids:
                    raise ValueError(
                        f"character placement references unavailable role: {placement.role_id}"
                    )
                if placement.role_id in placement_ids:
                    raise ValueError(f"duplicate character placement for role: {placement.role_id}")
                placement_ids.add(placement.role_id)
            if placement_ids != referenced_role_ids:
                raise ValueError(
                    "character_placements must exactly cover visually referenced character roles"
                )
            ...
            if entity_ids != placement_ids:
                raise ValueError(
                    "entity_states and character_placements must cover the same visible character roles"
                )
            for line in item.dialogue_lines:
                if (
    ...
    async def run(self, project_dir: Path, state: ProjectState) -> ProjectState:
        ...
        for episode_key in self.active_episode_keys(state):
            ...
            for clip_index, (_, clip) in enumerate(source_clips, start=1):
                ...
                rows = self._validate_model_output(
                    raw,
                    assets,
                    scene_id=clip.scene_id,
                    reference_budget=reference_budget,
                ...
                for row in rows:
                    if 1 + len(row.ref_ids) > reference_budget:
                        raise ValueError(
                            f"{clip_id} uses {1 + len(row.ref_ids)} total references; "
                    ...
                    ClipShotPlan(
                        ...
                        shots=[
                            ShotPlanItem(
                                ...
                                character_placements=item.character_placements,
                                camera=item.camera,
    ...
    async def run(self, project_dir: Path, state: ProjectState) -> ProjectState:
        ...
        for episode_key in self.active_episode_keys(state):
            ...
            retained = [
                ...
                if not self._force(self.workflow) and item.shot_id not in target_ids
            ]
                ...
                shot_rows = [
                    {
                        ...
                        "character_placements": [
                            ...
                            for placement in shot.character_placements
                        ],
...
class ShotBackgroundShotReferenceNode(ShotAssetNodeBase):
    name = "shot_background_shot_reference"
    ...
    async def run(self, project_dir: Path, state: ProjectState) -> ProjectState:
        ...
        if not bool(getattr(provider, "supports_reference_images", True)):
            raise ValueError("shot_background_shot_reference requires a reference-image provider")
        all_outputs: list[ShotBackgroundShotReferenceEpisodeOutput] = []
        for episode_key in self.active_episode_keys(state):
            plan = self._load_episode_output(project_dir, "clip_to_shots", episode_key, ClipToShotsEpisodeOutput)
            ...
            assets = self._all_assets(project_dir, state)
            path = self.layout.node_episode_output_path(project_dir, self.name, episode_key)
            existing = ShotBackgroundShotReferenceEpisodeOutput(episode_key=episode_key)
            if path.exists():
                existing = ShotBackgroundShotReferenceEpisodeOutput.model_validate_json(
                    path.read_text(encoding="utf-8")
                )
            by_shot = {item.shot_id: item for item in existing.generated_references}
            semaphore = asyncio.Semaphore(self._concurrency())

            async def generate(shot: ShotPlanItem) -> ShotBackgroundShotReferenceItem:
                scene_id, scene_item = self._shot_scene(shot, assets)
                scene = scene_item["object"]
                scene_ref = self._asset_ref(project_dir, scene_item)
                if not (scene_ref.path or scene_ref.url):
                    raise ValueError(f"scene {scene_id} has no usable spatial-anchor image")
                prompt = self.workflow.prompts.render(
                    ...
                    scene_description=self._clean(scene.desc or scene.name),
                    shot_description=shot.shot_description,
                    character_placements=json.dumps(
                        [item.model_dump(mode="json") for item in shot.character_placements],
                        ensure_ascii=False,
                        indent=2,
                    ),
                    camera=json.dumps(shot.camera.model_dump(mode="json"), ensure_ascii=False, indent=2),
                )
                fingerprint = self._generation_fingerprint(prompt, [scene_ref], provider)
                ...
                async with semaphore:
                    result, final_prompt, _ = await self._generate_image_with_safety_prompt_rewrites(
                        ...
                        state=state,
                        node_name=self.name,
                        asset_id=f"{shot.shot_id}_shot_reference",
                        prompt=prompt,
                        refs=[scene_ref],
                        metadata={
                            "node_name": self.name,
                            "prompt_asset_type": "shot_background_shot_reference",
                    ...
                    asset_path = await self.media_store.write_first_generated_image(
                        ...
                        self.layout.image_asset_path(
                            project_dir,
                            "shot_background_references",
                            shot.shot_id,
                        ),
                        result,
                    )
                return ShotBackgroundShotReferenceItem(
                    reference_id=f"{shot.shot_id}_shot_reference",
                    episode_key=episode_key,
            ...
            for shot in targets:
                ...
                prompt = self.workflow.prompts.render(
                    self.name,
                    scene_description=self._clean(scene_item["object"].desc or scene_item["object"].name),
                    shot_description=shot.shot_description,
                    character_placements=json.dumps(
                        [item.model_dump(mode="json") for item in shot.character_placements],
                        ensure_ascii=False,
            ...
            for item in generated:
                by_shot[item.shot_id] = item
            required_ids = {shot.shot_id for shot in targets}
            missing = required_ids.difference(by_shot)
            if missing:
                raise ValueError(
                    "shot_background_shot_reference missing shots: " + ", ".join(sorted(missing))
                )
            output = ShotBackgroundShotReferenceEpisodeOutput(
                episode_key=episode_key,
            ...
            self.repo.write_json(path, output)
            self._invalidate_shot_reference_dependents(
                project_dir,
                state,
                episode_key,
        ...
        self.repo.save_node_output(
            project_dir,
            self.name,
            ShotBackgroundShotReferenceOutput(episodes=all_outputs),
        )
        return state
...
class ShotBackgroundImageGenerationNode(ShotAssetNodeBase):
    ...
    async def run(self, project_dir: Path, state: ProjectState) -> ProjectState:
        ...
        for episode_key in self.active_episode_keys(state):
            ...
            reference_output = self._load_episode_output(
                project_dir,
                "shot_background_shot_reference",
                episode_key,
                ShotBackgroundShotReferenceEpisodeOutput,
            ...
            references_by_shot = {
                item.shot_id: item for item in reference_output.generated_references
            }
            assets = self._all_assets(project_dir, state)
            path = self.layout.node_episode_output_path(project_dir, self.name, episode_key)
            ...
            def generation_refs(item: ShotBackgroundPromptItem) -> list[AssetRef]:
                ...
                _scene_id, scene_item = self._shot_scene(shot, assets)
                reference = references_by_shot.get(shot.shot_id)
                if reference is None:
                    raise ValueError(f"Missing shot reference for {shot.shot_id}")
                if reference.scene_id != shot.scene_id:
                    raise ValueError(f"Shot reference scene mismatch for {shot.shot_id}")
                return [
                    self._asset_ref(project_dir, scene_item),
                    self._shot_reference_asset_ref(project_dir, reference),
                ]

            async def generate(item: ShotBackgroundPromptItem) -> ShotBackgroundImageGenerationItem:
                shot = shots_by_id[item.shot_id]
                shot_reference = references_by_shot[item.shot_id]
                ...
                return ShotBackgroundImageGenerationItem(
                    ...
                    shot_reference_id=shot_reference.reference_id,
                    shot_reference_fingerprint=shot_reference.fingerprint,
                    shot_reference_asset_path=shot_reference.asset_path,
                    shot_reference_asset_url=shot_reference.asset_url,
                    description=item.description,
                    prompt=final_prompt,
                    fingerprint=fingerprint,
                    asset_path=asset_path,
...
class ShotKeyframePromptNode(ShotAssetNodeBase):
    ...
    def _text_rendering_instruction(cls, shot: ShotPlanItem) -> str:
        if cls._requires_clean_plate(shot):
            return "Generate a clean plate with no readable text and preserve the specified area for post-production text."
        if shot.overlay_text_spec is not None:
            return f"Render this exact in-scene text: {shot.overlay_text_spec.text}"
        return "This shot has no exact text-rendering requirement."
    ...
    async def run(self, project_dir: Path, state: ProjectState) -> ProjectState:
        ...
        for episode_key in self.active_episode_keys(state):
            ...
            for background in backgrounds.generated_backgrounds:
                if background.shot_id in background_by_shot:
                    raise ValueError(f"{background.shot_id} has multiple generated backgrounds")
                background_by_shot[background.shot_id] = background
            path = self.layout.node_episode_output_path(project_dir, self.name, episode_key)
            ...
            for shot in targets:
                ...
                guide = ["Image 1: the generated shot background and locked camera anchor"]
                guide.extend(
                    f"Image {index}: {label}"
                    for index, (label, _) in enumerate(foreground, start=2)
                )
                request_prompt = self.workflow.prompts.render(
                    self.name,
                    shot_description=shot.shot_description,
                ...
                if clean_plate:
                    final_prompt += "\nThe image must be a clean plate with no readable text; exact text will be added in post-production."
                by_id[shot.shot_id] = ShotKeyframePromptItem(
                    ...
                    excluded_state_fields=["future_events", "future_prop_states", "exact_text"],
                    prompt_provenance={
                        "appearance_ids": [
            ...
            for row in rows:
                ...
                shots.append(
                    ShotManifestItem(
                        ...
                        character_placements=row.character_placements,
                        camera=row.camera,
                        camera_shooting_angle=row.camera.shooting_angle,
                        camera_movement=row.camera.movement,
                        focal_length=f"{row.camera.focal_length_mm:g} mm",
                        ...
                        input_fingerprints={
                            ...
                            "shot_reference": background.shot_reference_fingerprint,
                        },
...
def build_shot_asset_nodes(workflow: Any) -> list[WorkflowNode]:
    ...
    return [
        ...
        for cls in (
            ...
            ShotBackgroundShotReferenceNode,
