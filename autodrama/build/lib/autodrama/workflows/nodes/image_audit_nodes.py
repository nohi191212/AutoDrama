...
class LayoutImageAuditNode(ImageAuditNodeBase):
    ...
    def _asset_expectation(self, state: ProjectState, item: StaticAssetGenerationItem) -> str:
        ...
        return (
            f"A single 2:3 spatial-anchor sheet for the scene {layout.name}. {layout.desc}. "
            f"Fixed spatial anchors: {space}. It must show the same empty location in two vertically stacked, "
            "complementary high-angle isometric views with identical topology, entrances, structures, scale, "
            "materials, paths, north orientation, and lighting. No people, camera or character overlays, "
...
class ShotBackgroundImageAuditNode(ShotImageAuditNodeBase):
    ...
    def _asset_expectation_for_candidate(self, state: ProjectState, candidate: ShotImageAuditCandidate) -> str:
        return (
            ...
            "topology and the annotated shot reference's camera position, target, field of view, height, pitch, "
