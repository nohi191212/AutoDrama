...
class ShotPlanItem(BaseModel):
    ...
    @model_validator(mode="after")
    def derive_dialogue_display(self) -> "ShotPlanItem":
        expected = [line.text for line in self.dialogue_lines]
        if self.dialogue and self.dialogue != expected:
            raise ValueError("dialogue is display-only and must match dialogue_lines text")
        self.dialogue = expected
...
class ShotBackgroundShotReferenceItem(BaseModel):
    model_config = ConfigDict(extra="forbid")
...
class ShotBackgroundShotReferenceEpisodeOutput(BaseModel):
    model_config = ConfigDict(extra="forbid")

    episode_key: str
    generated_references: list[ShotBackgroundShotReferenceItem] = Field(default_factory=list)


class ShotBackgroundShotReferenceOutput(BaseModel):
    episodes: list[ShotBackgroundShotReferenceEpisodeOutput] = Field(default_factory=list)
...
class ShotBackgroundImageGenerationItem(BaseModel):
    ...
    shot_reference_fingerprint: str
    shot_reference_asset_path: str
    shot_reference_asset_url: str | None = None
    description: str
    prompt: str
    fingerprint: str
    asset_path: str
