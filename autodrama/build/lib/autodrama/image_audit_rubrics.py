...
# key_vision_image_audit is a world-anchor composition, overall figure/building
# proportion, spatial-reasoning, and visible rendering-cleanliness gate. It is
# deliberately not a detailed anatomy, lighting, text, or generic style gate.
KEY_VISION_AUDIT_DIMENSION_IDS = frozenset(
    {
        "figure_proportion",
        "continuous_space",
        "camera_anchor",
        "protagonist_hierarchy",
        "depth_hierarchy",
        "visual_hook",
        "lens_dof",
        "overhead_ground_order",
        "evidence_survival",
        "rendering_artifact_noise",
    }
)
...
def load_key_vision_audit_rubric_bundle(
    rubric_dir: str | Path | None = None,
) -> ProductionRubricBundle:
    """Load only the dimensions allowed to reject a key vision."""
    bundle = load_production_rubric_bundle(rubric_dir, include_xuanhuan_style=False)
    dimensions = [
        dimension
        for dimension in bundle.dimensions
        if dimension.id in KEY_VISION_AUDIT_DIMENSION_IDS
    ]
    missing = KEY_VISION_AUDIT_DIMENSION_IDS.difference({dimension.id for dimension in dimensions})
    if missing:
        raise ValueError(f"key vision audit rubric is missing dimensions: {sorted(missing)}")
