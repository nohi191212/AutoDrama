...
def test_deepseek_repairs_invalid_json_response(tmp_path, monkeypatch) -> None:
    calls = []
    responses = [
        '{"final_script":{"episode_001":"第一集内容"},"revision_notes":["补强爽点"}',
        '{"final_script":{"episode_001":"第一集内容"},"revision_notes":["补强爽点"]}',
    ]
    ...
    output = asyncio.run(
        provider.generate_json(
            "请打磨剧本。",
            ScriptPolishOutput,
            metadata={"node_name": "script_polish", "project_id": "test_project"},
        )
    )

    assert output.final_script["episode_001"] == "第一集内容"
    assert output.revision_notes == ["补强爽点"]
    assert len(calls) == 2
